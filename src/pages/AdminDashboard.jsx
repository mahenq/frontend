  ...
  const handleUpdate = async (id) => {
    try {
      ...
      await axios.put(
        ...
        {
          status: updates.status || customers.find((c) => c.id === id)?.status,
          notes: updates.notes || customers.find((c) => c.id === id)?.notes,
        },
        { headers: { Authorization: `Bearer ${token}` } }
      );
        ...
        <div className="content-wrapper">
          ...
          <div className="admin-container">
            <div className="admin-header">
              <div className="title admin-title">Admin Dashboard</div>
              <div className="subtitle admin-subtitle">
                Kelola pesan dari pengunjung website Anda. Anda dapat mengubah
                status, menambahkan catatan, dan menghapus pesan yang tidak
                diperlukan.
              </div>
            ...
            <div className="admin-stats">
              ...
              <div className="stat-item">
                <span className="stat-number">
                  {customers.filter((c) => c.status === "baru").length}
                </span>
                <span className="stat-label">Pesan Baru</span>
              </div>
              <div className="stat-item">
                <span className="stat-number">
                  {customers.filter((c) => c.status === "selesai").length}
                </span>
            ...
            <div className="admin-table">
              {customers.length === 0 ? (
                <div className="no-data">Tidak ada pesan dari pengunjung.</div>
              ) : (
                customers.map((cust) => (
                  <div className="admin-card" key={cust.id}>
                    <div className="admin-info">
                      ...
                      <p className="customer-email">{cust.email}</p>
                      <p className="customer-message">{cust.message}</p>
                      <p className="timestamp">
                        {new Date(cust.created_at).toLocaleString("id-ID")}
                      </p>
                      {cust.notes && (
                        <p className="customer-notes">
                          <strong>Catatan:</strong> {cust.notes}
                        </p>
                      )}
                    </div>
                    ...
                    <div className="admin-form">
                      <label>
                        ...
                        <select
                          value={
                            editingCustomer[cust.id]?.status || cust.status
                          }
                          onChange={(e) =>
                      ...
                      <label>
                        ...
                        <textarea
                          value={
                            editingCustomer[cust.id]?.notes || cust.notes || ""
                          }
                          onChange={(e) =>
                            handleInputChange(cust.id, "notes", e.target.value)
                          }
